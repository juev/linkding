...
func serveChangePassword(w http.ResponseWriter, r *http.Request, path string, cfg config.Config, db *sql.DB, users *auth.Repository) {
	...
	if r.Method == http.MethodPost {
		...
		if old == "" {
			data.OldError = "This field is required."
		} else if _, err := users.AuthenticatePassword(r.Context(), user.Username, old); errors.Is(err, auth.ErrInvalidCredentials) {
			data.OldError = "Your old password was entered incorrectly. Please enter it again."
		} else if err != nil {
			http.Error(w, "Server error", 500)
			return
		}
		if new1 == "" {
			data.NewError = "This field is required."
		}
		if new2 == "" {
