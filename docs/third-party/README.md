...
`internal/httpserver/admin_locale/` contains Django 6.0.7 compiled `django.mo` catalogs from `django/conf/locale`, `django/contrib/auth/locale`, and `django/contrib/admin/locale` in the same environment. The Go admin uses these catalogs for its login and dashboard translations. The catalogs carry the same BSD-3-Clause notice.
