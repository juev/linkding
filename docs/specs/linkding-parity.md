...
- R7. All ordinary and admin pages use the appearance, CSS, images, themes, and interactions of linkding v1.47.0. Admin pages select translations from the `ld_language` cookie, then `Accept-Language`, with English as the fallback; page text, labels, and language metadata match the pinned Django server. Turbo Frame and Stream behavior, Lit components, forms, shortcuts, custom CSS, and responsive layout remain available. Preserve the original MIT license and third-party asset notices.
