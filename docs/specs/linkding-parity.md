...
- R1. A bookmark can be created, edited, deleted, archived, restored, marked as read, shared, and changed in bulk through the UI and API. On creation, an existing bookmark owned by the user is found by normalized URL and follows the update path; when changing a URL, the duplicate check uses the exact string. Tag changes, timestamps, and background jobs follow the corresponding upstream path. Validation and authorization errors leave state unchanged.
